...
class PaymentEdit extends Component {
  ...
  componentDidMount() {
    ...
    //   });
    //   this.setState({ paymentEditData: prop_data });
    // console.log("params", this.props.match.params.id);
    this.setState({ paymentEditData: this.props.match.params.id });
    this.initRows();
  }
  ...
  setPaymentEditData = () => {
    ...
    console.log("id of payment edit", this.state.paymentEditData);
    formData.append("payment_id", this.state.paymentEditData);
    get_Payment_by_id(formData)
      .then((response) => {
        let res = response.data;
        console.log("res", res);
        if (res.responseStatus == 200) {
          let { payment_peritculars } = res;
          const {
            purchaseAccLst,
            supplierNameLst,
            supplierCodeLst,
          ...
          this.myRef.current.setFieldValue("payment_code", res.payment_no);
          ...
          console.log("peritculars", res.payment_peritculars);
          let initRowData = [];

          if (payment_peritculars.length > 0) {
            payment_peritculars.map((v) => {
              console.log("==='''vvvv", v);
              // ;
              let per = "";
              if (v.type == "dr") {
              ...
              let inner_data = {
                ...
                debit: v.type == "dr" ? v.dr : "",
                credit: v.type == "cr" ? v.cr : "",
                narration: "",
              };
  ...
  render() {
    const { isLoading, initVal, rows, cashAcbankLst } = this.state;
...
export default WithUserPermission(PaymentEdit);
