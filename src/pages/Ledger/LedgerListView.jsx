...
function Attendance2(props) {
  ...
  const onPuncIntModalShow = (status, attObject) => {
    console.log({ attObject });
    if (attObject.transaction_type === 5) {
      props.history.push("/master/receipt-edit/" + attObject.invoice_id);
    } else if (attObject.transaction_type === 6) {
      props.history.push("/master/payment-edit/" + attObject.invoice_id);
    } else if (attObject.transaction_type === 9) {
      props.history.push("/master/contra-edit/" + attObject.invoice_id);
    } else if (attObject.transaction_type === 10) {
      props.history.push("/master/journal-edit/" + attObject.invoice_id);
    }
    // }
    // if (status == true) {
    //   let request_data = {
  ...
  const handleCheckboxSelection = (e, id, index) => {
    const { name, checked } = e.target; // Use "checked" instead of "value" for checkboxes
    const list = [...CheckList];
    console.log("in group  CheckList-->", CheckList);

    // Update the specific object in the array
    list[index] = {
      ...list[index],
      id, // Bind the 'id'
      [name]: checked, // Bind the 'checked' value
    };
            ...
            <Table bordered size="sm" className="main-tbl-style">
              ...
              >
                <tr>
                  ...
                  <th className="th-style">Voucher No</th>
                  <th className="th-style">Voucher Type</th>
                  <th className="th-style">Perticular</th>
                  <th className="th-style">Debit</th>
                  <th className="th-style">Credit</th>
                  ...
                  {/* <th className="th-style">Punch In Selfie</th>
                  <th className="th-style">Punch Out Selfie</th> */}
